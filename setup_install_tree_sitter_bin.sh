...
TARGET_BIN_PATH="${TARGET_BIN_DIR}/install_tree_sitter_bin.sh"

BASH_COMPLETION_DIR="${HOME}/.scripts/completions/bash"
BASH_COMPLETION_PATH="${BASH_COMPLETION_DIR}/install_tree_sitter_bin.bash"

ZSH_COMPLETION_DIR="${HOME}/.scripts/completions/zsh"
ZSH_COMPLETION_FILE="${ZSH_COMPLETION_DIR}/install_tree_sitter_bin.zsh"
ZSH_COMPLETION_FUNC="${ZSH_COMPLETION_DIR}/_install_tree_sitter_bin"
...
BASH_COMPLETION_MARKER="# Added by ${SCRIPT_NAME} for install_tree_sitter_bin bash completion"
BASH_COMPLETION_LINE='source "$HOME/.scripts/completions/bash/install_tree_sitter_bin.bash"'

ZSH_FPATH_MARKER="# Added by ${SCRIPT_NAME} for install_tree_sitter_bin zsh completion"
ZSH_FPATH_LINE='fpath=("$HOME/.scripts/completions/zsh" $fpath)'
ZSH_COMPINIT_LINE='autoload -Uz compinit && compinit -i'
ZSH_COMPLETION_SOURCE_LINE='source "$HOME/.scripts/completions/zsh/install_tree_sitter_bin.zsh"'
...
Installs:
  - install_tree_sitter_bin.sh to \$HOME/.scripts
...
It also ensures:
  ...
  - install_tree_sitter_bin.zsh is sourced from ~/.zshrc
EOF
}
...
copy_script_assets() {
  require_file "${SOURCE_DIR}/install_tree_sitter_bin.sh"
  require_file "${SOURCE_DIR}/install_tree_sitter_bin.bash"
  require_file "${SOURCE_DIR}/install_tree_sitter_bin.zsh"

  mkdir -p "$TARGET_BIN_DIR" "$BASH_COMPLETION_DIR" "$ZSH_COMPLETION_DIR"

  install -m 0755 "${SOURCE_DIR}/install_tree_sitter_bin.sh" "$TARGET_BIN_PATH"
  install -m 0644 "${SOURCE_DIR}/install_tree_sitter_bin.bash" "$BASH_COMPLETION_PATH"
  install -m 0644 "${SOURCE_DIR}/install_tree_sitter_bin.zsh" "$ZSH_COMPLETION_FILE"
  install -m 0644 "${SOURCE_DIR}/install_tree_sitter_bin.zsh" "$ZSH_COMPLETION_FUNC"
