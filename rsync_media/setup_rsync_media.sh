...
TARGET_BIN_PATH="${TARGET_BIN_DIR}/rsync_media.sh"
TARGET_BIN_LINK="${TARGET_BIN_DIR}/rsync-media"

BASH_COMPLETION_DIR="${HOME}/.scripts/completions/bash"
BASH_COMPLETION_PATH="${BASH_COMPLETION_DIR}/rsync_media.bash"

ZSH_COMPLETION_DIR="${HOME}/.scripts/completions/zsh"
ZSH_COMPLETION_FILE="${ZSH_COMPLETION_DIR}/rsync_media.zsh"
ZSH_COMPLETION_FUNC="${ZSH_COMPLETION_DIR}/_rsync_media"
...
BASH_COMPLETION_MARKER="# Added by ${SCRIPT_NAME} for rsync_media bash completion"
BASH_COMPLETION_LINE='source "$HOME/.scripts/completions/bash/rsync_media.bash"'

ZSH_FPATH_MARKER="# Added by ${SCRIPT_NAME} for rsync_media zsh completion"
ZSH_FPATH_LINE='fpath=("$HOME/.scripts/completions/zsh" $fpath)'
ZSH_COMPINIT_LINE='autoload -Uz compinit && compinit -i'
ZSH_COMPLETION_SOURCE_LINE='source "$HOME/.scripts/completions/zsh/rsync_media.zsh"'
...
Installs:
  - rsync_media.sh to \$HOME/.scripts
  - rsync-media symlink to \$HOME/.scripts
...
It also ensures:
  ...
  - rsync_media.zsh is sourced from ~/.zshrc
EOF
}
...
copy_script_assets() {
  require_file "${SOURCE_DIR}/rsync_media.sh"
  require_file "${SOURCE_DIR}/rsync_media.bash"
  require_file "${SOURCE_DIR}/rsync_media.zsh"

  mkdir -p "$TARGET_BIN_DIR" "$BASH_COMPLETION_DIR" "$ZSH_COMPLETION_DIR"

  install -m 0755 "${SOURCE_DIR}/rsync_media.sh" "$TARGET_BIN_PATH"
  ln -sf "$TARGET_BIN_PATH" "$TARGET_BIN_LINK"

  install -m 0644 "${SOURCE_DIR}/rsync_media.bash" "$BASH_COMPLETION_PATH"
  install -m 0644 "${SOURCE_DIR}/rsync_media.zsh" "$ZSH_COMPLETION_FILE"
  install -m 0644 "${SOURCE_DIR}/rsync_media.zsh" "$ZSH_COMPLETION_FUNC"

  log_success "Installed script to $(color_text value "$TARGET_BIN_PATH")"
  log_success "Installed symlink to $(color_text value "$TARGET_BIN_LINK")"
