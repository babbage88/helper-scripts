...
TARGET_BIN_PATH="${TARGET_BIN_DIR}/add-routes.sh"

BASH_COMPLETION_DIR="${HOME}/.scripts/completions/bash"
BASH_COMPLETION_PATH="${BASH_COMPLETION_DIR}/add-routes.bash"
BASH_COMPLETION_SCRIPT_COPY="${BASH_COMPLETION_DIR}/add-routes.sh"

ZSH_COMPLETION_DIR="${HOME}/.scripts/completions/zsh"
ZSH_COMPLETION_FILE="${ZSH_COMPLETION_DIR}/add-routes.zsh"
ZSH_COMPLETION_FUNC="${ZSH_COMPLETION_DIR}/_add_routes"
ZSH_COMPLETION_SCRIPT_COPY="${ZSH_COMPLETION_DIR}/add-routes.sh"
...
BASH_COMPLETION_MARKER="# Added by ${SCRIPT_NAME} for add-routes bash completion"
BASH_COMPLETION_LINE='source "$HOME/.scripts/completions/bash/add-routes.bash"'

ZSH_FPATH_MARKER="# Added by ${SCRIPT_NAME} for add-routes zsh completion"
ZSH_FPATH_LINE='fpath=("$HOME/.scripts/completions/zsh" $fpath)'
ZSH_COMPINIT_LINE='autoload -Uz compinit && compinit -i'
ZSH_COMPLETION_SOURCE_LINE='source "$HOME/.scripts/completions/zsh/add-routes.zsh"'
...
It also ensures:
  ...
  - add-routes.zsh is sourced from ~/.zshrc

Note:
  The add-routes completion scripts parse ROUTE_ENTRIES from add-routes.sh, so this
  installer also places sidecar copies of add-routes.sh next to each completion file.
EOF
}
...
copy_script_assets() {
  require_file "${SOURCE_DIR}/add-routes.sh"
  require_file "${SOURCE_DIR}/add-routes.bash"
  require_file "${SOURCE_DIR}/add-routes.zsh"

  mkdir -p "$TARGET_BIN_DIR" "$BASH_COMPLETION_DIR" "$ZSH_COMPLETION_DIR"

  install -m 0755 "${SOURCE_DIR}/add-routes.sh" "$TARGET_BIN_PATH"

  install -m 0644 "${SOURCE_DIR}/add-routes.bash" "$BASH_COMPLETION_PATH"
  install -m 0644 "${SOURCE_DIR}/add-routes.sh" "$BASH_COMPLETION_SCRIPT_COPY"

  install -m 0644 "${SOURCE_DIR}/add-routes.zsh" "$ZSH_COMPLETION_FILE"
  install -m 0644 "${SOURCE_DIR}/add-routes.zsh" "$ZSH_COMPLETION_FUNC"
  install -m 0644 "${SOURCE_DIR}/add-routes.sh" "$ZSH_COMPLETION_SCRIPT_COPY"

  log_success "Installed script to $(color_text value "$TARGET_BIN_PATH")"
  log_success "Installed Bash completion files to $(color_text value "$BASH_COMPLETION_DIR")"
  log_success "Installed Zsh completion files to $(color_text value "$ZSH_COMPLETION_DIR")"
}
