...
TARGET_BIN_PATH="${TARGET_BIN_DIR}/tailscale_client.sh"

BASH_COMPLETION_DIR="${HOME}/.scripts/completions/bash"
BASH_COMPLETION_PATH="${BASH_COMPLETION_DIR}/tailscale_client.bash"

ZSH_COMPLETION_DIR="${HOME}/.scripts/completions/zsh"
ZSH_COMPLETION_FILE="${ZSH_COMPLETION_DIR}/tailscale_client.zsh"
ZSH_COMPLETION_FUNC="${ZSH_COMPLETION_DIR}/_tailscale_client"
...
BASH_COMPLETION_MARKER="# Added by ${SCRIPT_NAME} for tailscale_client bash completion"
BASH_COMPLETION_LINE='source "$HOME/.scripts/completions/bash/tailscale_client.bash"'

ZSH_FPATH_MARKER="# Added by ${SCRIPT_NAME} for tailscale_client zsh completion"
ZSH_FPATH_LINE='fpath=("$HOME/.scripts/completions/zsh" $fpath)'
ZSH_COMPINIT_LINE='autoload -Uz compinit && compinit -i'
ZSH_COMPLETION_SOURCE_LINE='source "$HOME/.scripts/completions/zsh/tailscale_client.zsh"'
...
Installs:
  - tailscale_client.sh to \$HOME/.scripts
...
It also ensures:
  ...
  - tailscale_client.zsh is sourced from ~/.zshrc
EOF
}
...
copy_script_assets() {
  require_file "${SOURCE_DIR}/tailscale_client.sh"
  require_file "${SOURCE_DIR}/tailscale_client.bash"
  require_file "${SOURCE_DIR}/tailscale_client.zsh"

  mkdir -p "$TARGET_BIN_DIR" "$BASH_COMPLETION_DIR" "$ZSH_COMPLETION_DIR"

  install -m 0755 "${SOURCE_DIR}/tailscale_client.sh" "$TARGET_BIN_PATH"
  install -m 0644 "${SOURCE_DIR}/tailscale_client.bash" "$BASH_COMPLETION_PATH"
  install -m 0644 "${SOURCE_DIR}/tailscale_client.zsh" "$ZSH_COMPLETION_FILE"
  install -m 0644 "${SOURCE_DIR}/tailscale_client.zsh" "$ZSH_COMPLETION_FUNC"
